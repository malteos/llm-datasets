...
id_to_dataset_class = {
    cls.DATASET_ID: cls
    for cls in get_registered_dataset_classes(extra_dataset_registries=args.extra_dataset_registries)
}
...
# @st.cache_data
def load_data():
    if args.dataframe_cache_path is None or (
        args.dataframe_cache_path is not None and not os.path.exists(args.dataframe_cache_path)
    ):
        ...
        # load from disk
        logger.info("Reading df from disk: %s", args.dataframe_cache_path)
        _df = pd.read_csv(args.dataframe_cache_path, index_col=None)

    # save to disk?
    if args.dataframe_cache_path and not os.path.exists(args.dataframe_cache_path):
        logger.info("Writing df to disk: %s", args.dataframe_cache_path)
        _df.to_csv(args.dataframe_cache_path, index=False)
...
if mode == "overview":
    ...
    if ds.has_output_files(min_file_size=271):  # empty parquet files have 270 bytes
        st.header("Text preview:")

        TEXT_COLUMN_NAME = "text"
        MAX_TEXT_LENGTH = 50_000

        pq_fps = list(sorted(ds.get_output_file_paths()))

        pq_fp = str(pq_fps[0])  # using only part 1 if multiple chunks
        ...
        st.markdown(f"The dataset's output file does not exist (or is empty): `{ds.get_output_file_paths()}`")
